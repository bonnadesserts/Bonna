...
const STORAGE_KEYS = {
  products: 'bonnago_products_v3',
  categories: 'bonnago_categories_v2',
  ...
  users: 'bonnago_users_v1',
  userSession: 'bonnago_user_session_v1',
  googleClientId: 'bonnago_google_client_id_v1',
};

const DEFAULT_CATEGORIES = [
  { id: 'cat-tortas',    name: 'Tortas',    emoji: '🎂', allowsSizes: true,  minDays: 2 },
  { id: 'cat-porciones', name: 'Porciones', emoji: '🍰', allowsSizes: false, minDays: 0 },
  { id: 'cat-jars',      name: 'Jars',      emoji: '🍮', allowsSizes: true,  minDays: 0 },
];
...
const DEFAULT_PRODUCTS = [
  {
    ...
    discount: 0,
    emoji: '🥕',
    description: 'Húmeda, especiada, con frosting de queso crema. Ideal para celebrar.',
    allowsToppings: false,
    toppingIds: [],
    sizes: [
      { id: 's-pq', name: 'Pequeña (8 porc.)',  price: 65000 },
      { id: 's-md', name: 'Mediana (12 porc.)', price: 95000 },
      { id: 's-gd', name: 'Grande (20 porc.)',  price: 145000 },
    ],
    tag: 'Favorita',
  },
  {
    ...
    discount: 0,
    emoji: '🍫',
    description: 'Bizcocho de chocolate intenso con ganache sedoso. Listo en tu frasco.',
    allowsToppings: true,
    toppingIds: ['t-fresa', 't-arequipe', 't-oreo'],
    sizes: [
      { id: 's-8oz',  name: '8 oz',  price: 14000 },
      { id: 's-12oz', name: '12 oz', price: 18000 },
      { id: 's-16oz', name: '16 oz', price: 22000 },
    ],
    tag: 'Más vendido',
  },
  {
    ...
    toppingIds: [],
    sizes: [
      { id: 's-md', name: 'Mediana (10 porc.)', price: 72000 },
      { id: 's-gd', name: 'Grande (16 porc.)',  price: 115000 },
    ],
    tag: '10% OFF',
  },
  {
  ...
  {
    id: 'p-5',
    name: 'Porción de brownie con helado',
    categoryId: 'cat-porciones',
    price: 12000,
    discount: 0,
    emoji: '🍫',
  ...
  },
  {
    ...
    discount: 0,
    emoji: '🍰',
    description: 'Una porción individual del cheesecake de frutos rojos. Lista para ti.',
    allowsToppings: false,
    toppingIds: [],
    sizes: [],
    tag: null,
  },
  {
    ...
    discount: 0,
    emoji: '🍫',
    description: 'Húmeda, intensa, con ganache. Lista para comer.',
    allowsToppings: false,
    toppingIds: [],
    sizes: [],
    tag: null,
  },
  {
    ...
    discount: 0,
    emoji: '🍪',
    description: 'Capas de bizcocho cremoso con galleta oreo crocante en cada bocado.',
    allowsToppings: true,
    toppingIds: ['t-malv', 't-crema'],
    sizes: [
      { id: 's-8oz',  name: '8 oz',  price: 15000 },
      { id: 's-12oz', name: '12 oz', price: 19000 },
    ],
    tag: null,
  },
];
...
const DataAPI = {
  ...
  getUsers()       { return Store.get(STORAGE_KEYS.users, []); },
  setUsers(u)      { Store.set(STORAGE_KEYS.users, u); },
  getUserSession() { return Store.get(STORAGE_KEYS.userSession, null); },
  setUserSession(s){ Store.set(STORAGE_KEYS.userSession, s); },
  clearUserSession(){ Store.remove(STORAGE_KEYS.userSession); },

  registerUser({ name, email, phone, password, provider = 'email', googleId = null, avatar = '' }) {
    const users = this.getUsers();
    const norm = email.trim().toLowerCase();
    const existing = users.find(u => u.email.toLowerCase() === norm);
    if (existing) {
      // Si existía via Google sin password, completa
      return { ok: false, reason: 'exists', user: existing };
    }
    const user = {
  ...
  loginUser(email, password) {
    const norm = (email || '').trim().toLowerCase();
    const user = this.getUsers().find(u => u.email.toLowerCase() === norm);
    if (!user) return { ok: false, reason: 'not_found' };
    if (user.provider === 'google' && !user.password) return { ok: false, reason: 'use_google' };
    if (user.password !== password) return { ok: false, reason: 'wrong_password' };
    return { ok: true, user };
  },

  loginOrRegisterGoogle({ googleId, email, name, avatar }) {
    const users = this.getUsers();
    const norm = (email || '').trim().toLowerCase();
    let user = users.find(u => u.email.toLowerCase() === norm);
    if (user) {
      // upsert info de google
      user.googleId = googleId;
      user.avatar = avatar || user.avatar;
      if (!user.provider || user.provider === 'email') user.provider = user.password ? 'email' : 'google';
      this.setUsers(users);
    } else {
      user = {
        id: 'user-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6),
  ...
  deleteUser(id) {
    this.setUsers(this.getUsers().filter(u => u.id !== id));
  },
  ...
  getOrdersByEmailOrPhone(email, phone) {
    const e = (email || '').trim().toLowerCase();
    const p = (phone || '').trim();
    return this.getOrders().filter(o => {
      const oe = (o.customer?.email || '').trim().toLowerCase();
      const op = (o.customer?.phone || '').trim();
      return (e && oe === e) || (p && op === p);
    });
  },

  // Google Client ID configurable desde admin
  getGoogleClientId() { return Store.get(STORAGE_KEYS.googleClientId, ''); },
  setGoogleClientId(v) { Store.set(STORAGE_KEYS.googleClientId, v || ''); },
